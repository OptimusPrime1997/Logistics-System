...
public class PayRepVO extends ReceiptVO {
	...
	public PayRepVO(String num, String date, double sum, ArrayList<PayVO> payVOs, String payPersonNum,
			ArrayList<PayRepRefundRepVO> refund, PayRepStaffSalaryRepVO staffSalary, PayRepFreightRepVO freight,
			PayRepRentRepVO rent, PayRepDriverSalaryRepVO driverSalary, PayRepCourierSalaryRepVO courierSalary,
			PayRepBonusRepVO bonus) {
		super(num, date);
		this.sum = sum;
		if (payVOs == null) {
			this.payVOs = new ArrayList<PayVO>();
		} else {
			this.payVOs = payVOs;
		}
		this.payPersonNum = payPersonNum;
		if (refund == null) {
			this.refund = new ArrayList<PayRepRefundRepVO>();
		} else {
			this.refund = refund;
		}
